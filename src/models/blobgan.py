...
class BlobGAN(BaseModule):
    ...
    def configure_optimizers(self) -> Union[optim, List[optim]]:
        ...
        def req_grad(l): return [p for p in l if p.requires_grad]
        decay_params = []
        ...
        G_optim = torch.optim.AdamW(
            G_params, lr=self.lr * G_reg_ratio,
            betas=(0 ** G_reg_ratio, 0.99 ** G_reg_ratio),
            eps=self.eps, weight_decay=0)
        D_optim = torch.optim.AdamW(
            D_params, lr=self.lr * D_reg_ratio,
            betas=(0 ** D_reg_ratio, 0.99 ** D_reg_ratio),
            eps=self.eps, weight_decay=0)
        print_once(
            f'Optimizing {sum([p.numel() for grp in G_params for p in grp["params"]]) / 1e6:.2f}M params for G '
    ...
    def gen(self, z=None, layout=None, ema=False, norm_img=False,
            ret_layout=False, ret_latents=False, noise=None, **kwargs):
        assert (z is not None) or (layout is not None)
        if layout is not None and 'covs_raw' not in kwargs:
            kwargs['covs_raw'] = False
        layout = self.generate_layout(
            z, layout=layout, ret_layout=ret_layout, ema=ema, **kwargs)
        gen_input = {
            'input': layout['feature_grid'],
            'styles': {k: layout[k] for k in SPLAT_KEYS}
            if self.spatial_style else z, 'return_image_only': not ret_latents,
            'return_latents': ret_latents, 'noise': noise}
    ...
    def log_fid(self, mode, **kwargs):
        def gen_fn(z):
            return self.gen(
                z, ema=self.accumulate, norm_img=True).clamp(
                min=0, max=255)

        if is_rank_zero():
            fid_score = fid.compute_fid(
                gen=gen_fn, dataset_name=self.fid_stats_name,
    ...
    def visualize_features(self, xs, ys, viz_size, features=None, scores=None, feature_img=None,
        ...
        if feature_img is None:
            ...
            viz_colors = (
                viz_colors
                if not rand_colors else torch.rand_like(features[..., : 3])).to(
                xs.device, non_blocking=True)
            if viz_colors.ndim == 2:
                # viz colors should be [Kmax, 3]
                viz_colors = viz_colors[:features.size(
                    1)][None].repeat_interleave(len(features), 0)
            ...
            if rand_colors:
                ...
                feature_img = img.sub(imin).div(
                    (imax - imin).clamp(min=1e-5)).mul(2).sub(1)
            else:
                feature_img = img
        ...
        if viz_centers:
            ...
            if feature_center_mask is not None:
                fill_center = centers[torch.arange(
                    len(centers)), feature_center_mask.int().argmax(1)]
                centers[~feature_center_mask] = fill_center.repeat_interleave(
                    (~feature_center_mask).sum(1), dim=0)
            offsets = (-sz // 2, sz // 2 + 1)
            offsets = (torch.arange(*offsets)
                       [None] + torch.arange(*offsets).mul(viz_size)[:, None])
            ...
            # draw center
            pixels = (
                centers[..., None] + offsets_center[None, None].to(
                    self.device, non_blocking=True)).clamp(
                min=0, max=imgs_flat.size(-1) - 1)
            ...
            # draw borders
            pixels = (
                centers[..., None] + offsets_border[None, None].to(
                    self.device, non_blocking=True)).clamp(
                min=0, max=imgs_flat.size(-1) - 1)
            pixels = pixels.flatten(start_dim=1)
            pixels = repeat(pixels, 'n m -> n c m', c=3)
        ...
        if scores is not None and viz_entropy:
            ...
            img = img.sub(imin).div(
                (imax - imin).clamp(min=1e-5)).mul(256).int().cpu()
            h = w = img.size(-1)
            img = torch.from_numpy(
                cm.plasma(img.flatten())).mul(2).sub(1)[:, :-1]
            out['entropy_img'] = rearrange(
                img, '(n h w) c -> n c h w', h=h, w=w)
        return out

    def splat_features(
            self, xs: Tensor, ys: Tensor, features: Tensor, covs: Tensor,
            sizes: Tensor, size: int, score_size: int,
            viz_size: Optional[int] = None, viz: bool = False, ret_layout:
            bool = True, covs_raw: bool = True, pyramid: bool = True,
            no_jitter: bool = False, no_splat: bool = False, viz_score_fn=None, **
            kwargs) -> Dict:
        """
        Args:
        ...
        if self.feature_jitter_xy and not no_jitter:
            xs = xs + torch.empty_like(xs).uniform_(-self.feature_jitter_xy,
                                                    self.feature_jitter_xy)
            ys = ys + torch.empty_like(ys).uniform_(-self.feature_jitter_xy,
                                                    self.feature_jitter_xy)
        if covs_raw:
            ...
            if self.feature_jitter_angle and not no_jitter:
                with torch.no_grad():
                    ...
                    theta = theta + torch.empty_like(
                        theta).uniform_(-self.feature_jitter_angle, self.feature_jitter_angle)
                    basis_i_jitter = (rotation_matrix(
                        theta)[..., 0] - basis_i).detach()
            ...
            covs = covs + torch.eye(2)[None, None].to(covs.device,
                                                      non_blocking=True) * 1e-5
        ...
        grid_coords = torch.stack(
            (torch.arange(score_size).repeat(score_size),
             torch.arange(score_size).repeat_interleave(score_size))).to(
            xs.device)  # [2, size*size]
        delta = (grid_coords[None, None] - feature_coords[...,
                 None]).div(score_size)  # [n, m, 2, size*size]

        sq_mahalanobis = (delta * torch.linalg.solve(covs, delta)).sum(2)
        sq_mahalanobis = einops.rearrange(
            sq_mahalanobis, 'n m (s1 s2) -> n s1 s2 m', s1=score_size)

        # [n, h, w, m]
        shift = sizes[:, None, None, 1:]
        if self.feature_jitter_shift and not no_jitter:
            shift = shift + torch.empty_like(
                shift).uniform_(-self.feature_jitter_shift, self.feature_jitter_shift)
        ...
        d_scores = (1 - scores[..., rev]).cumprod(-1)[...,
                                                      rev].roll(-1, -1) * scores
        d_scores[..., -1] = scores[..., -1]
        ...
        feature_grid = splat_features_from_scores(
            ret['scores_pyramid'][size],
            features, size, channels_last=False)
        ret.update({'feature_grid': feature_grid,
                   'feature_img': None, 'entropy_img': None})
        if ret_layout:
            layout = {'xs': xs, 'ys': ys, 'covs': covs, 'raw_scores': scores,
                      'sizes': sizes, 'composed_scores': d_scores, 'features': features}
            ret.update(layout)
        if viz:
            if viz_score_fn is not None:
                viz_posterior = viz_score_fn(scores)
                scores_viz = (
                    1 - viz_posterior[..., rev]).cumprod(-1)[..., rev].roll(-1, -1) * viz_posterior
                scores_viz[..., -1] = viz_posterior[..., -1]
            else:
                scores_viz = d_scores
            ret.update(self.visualize_features(
                xs, ys, viz_size, features, scores_viz, **kwargs))
        return ret

    def generate_layout(self, z: Optional[Tensor] = None, ret_layout: bool = False,
                        ema: bool = False, size: Optional[int] = None,
                        viz: bool = False, num_features: Optional[int] = None,
                        layout: Optional[Dict[str, Tensor]] = None,
                        mlp_idx: Optional[int] = None,
                        score_size: Optional[int] = None,
                        viz_size: Optional[int] = None,
                        truncate: Optional[float] = None, **kwargs) -> Dict[str,
                                                                            Tensor]:
        """
        Args:
        ...
        if num_features is None:
            num_features = random.randint(
                self.n_features_min, self.n_features_max)
        if layout is None:
        ...
        ret = self.splat_features(
            **layout, size=size or G.size_in, viz_size=viz_size or G.size,
            viz=viz, ret_layout=ret_layout,
            score_size=score_size or (size or G.size),
            pyramid=True, **kwargs)
    ...
    def get_mean_latent(self, n_trunc: int = 10000, ema=True):
        ...
        layout_net = self.layout_net_ema if ema else self.layout_net
        latents = [
            layout_net.mlp[: -1](Z[_])
            for _ in trange(n_trunc, desc='Computing mean latent')]
    ...
    def shared_step(self, batch: Tuple[Tensor, dict],
        ...
        """
        if run_at_step(
                self.trainer.global_step, self.flush_cache_every_n_steps):
        ...
        log_images = run_at_step(
            self.trainer.global_step, self.log_images_every_n_steps)
        layout, gen_imgs, latents = self.gen(
            z, ret_layout=True, ret_latents=True, viz=log_images)
        ...
        if train_G or not train:
            ...
            if run_at_step(
                    self.trainer.global_step, self.trainer.log_every_n_steps):
                with torch.no_grad():
                    ...
                    info.update({'coord_spread': (coords - centroids)
                                [coord_mask].norm(2, -1).mean()})
            ...
            if train_G:
                if self.accumulate:
                    accumulate(self.generator_ema, self.generator,
                               0.5 ** (32 / (10 * 1000)))
                    accumulate(self.layout_net_ema, self.layout_net,
                               0.5 ** (32 / (10 * 1000)))
                if log_images and is_rank_zero():
                    if self.accumulate and self.n_ema_sample:
                        with torch.no_grad():
                            z = self.sample_z.to(self.device, non_blocking=True)
                            layout, imgs['gen_imgs_ema'] = self.gen(
                                z, ema=True, viz=True, ret_layout=True)
                            imgs['feature_imgs_ema'] = layout['feature_img']
                    imgs = {k: v.clone().detach().float().cpu()
                            for k, v in imgs.items() if v is not None}
                    self._log_image_dict(
                        imgs, mode, square_grid=False, ncol=len(batch_real))
                if run_at_step(self.trainer.global_step, self.
                               log_fid_every_n_steps) and train_G:
                    self.log_fid(mode)
                self._log_profiler()
