...
class BlobGANInverter(BaseModule):
    ...
    def __post_init__(self):
        ...
        if not self.load_only_inverter:
            self.generator = load_pretrained_weights(
                'BlobGAN', self.generator_pretrained, BlobGAN(**cfg), strict=False)
    ...
    def configure_optimizers(self) -> Union[optim, List[optim]]:
        ...
        print_once(
            f'Optimizing {sum([p.numel() for p in params]) / 1e6:.2f}M params')
        return torch.optim.AdamW(
            params, lr=self.lr, eps=self.eps, weight_decay=0)
    ...
    def shared_step(self, batch: Tuple[Tensor, dict],
        ...
        z = torch.randn(
            len(batch_real),
            self.generator.noise_dim).type_as(batch_real)

        with torch.no_grad():
            truncate = self.trunc_min if self.trunc_min == self.trunc_max \
                else random.uniform(self.trunc_min, self.trunc_max)
            layout_gt_fake, gen_imgs = self.generator.gen(
                z, truncate=truncate, ema=True, viz=log_images, ret_layout=True)

        losses = dict()

        z_pred_fake = self.inverter(gen_imgs.detach())

        layout_pred_fake, reconstr_fake = self.generator.gen(
            z_pred_fake, ema=True, viz=log_images, ret_layout=True,
        ...
        for k in ('xs', 'ys', 'covs', 'sizes', 'features', 'spatial_style'):
            latent_l2_loss.append(
                (layout_pred_fake[k] - layout_gt_fake[k].detach()).pow(2).mean())
        losses['fake_latents_MSE'] = sum(latent_l2_loss) / len(latent_l2_loss)

        z_pred_real = self.inverter(batch_real.detach())
        layout_pred_real, reconstr_real = self.generator.gen(
            z_pred_real, ema=True, viz=log_images, ret_layout=True,
            mlp_idx=len(self.generator.layout_net_ema.mlp))
